...
Due to [Gatsby's server-side rendering](#gatsby-ssr) mechanism, standard methods for determining a user's device type in JavaScript were suboptimal for the static site I was building.
...
For the simplicity's sake, we'll define an array of objects in code that include statically imported images (rather than playing around with a GraphQL query):
...
The slightly unusual CSS syntax (e.g., `.image.image1`) is making use of [inheritance](#css-inheritance) to reduce code duplication.

## Notes

### <a name="gatsby-ssr"></a> Gatsby's static server-side rendering
...
### <a name="css-inheritance"></a> CSS inheritance
